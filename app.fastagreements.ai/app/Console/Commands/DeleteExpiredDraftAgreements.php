 ...
 * photos) live on S3 (see app/Traits/ImageResizer.php) and are deleted from
 * there first, then the row itself; agreement_attribute rows are removed
 * automatically via the DB's cascadeOnDelete on agreement_id.
 *
...
class DeleteExpiredDraftAgreements extends Command
{
    ...
    private function deleteDraftImages(Aggriment $draft): void
    {
        ...
        foreach (self::IMAGE_FOLDERS as $field => $folder) {
            ...
            $original = "admin/images/{$folder}/{$filename}";
            $thumb    = "admin/images/{$folder}_thumb/{$filename}";
