...
<main id="main" class="main">
    ...
    <div class="row">
        <div class="pagetitle col-lg-6 pt-2">
            ...
            <nav>
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item"><a href="{{ route('deal_categories.index') }}">Agreement Categories</a></li>
        ...
        <div class="pagetitle col-lg-6 text-end pt-2">
            ...
            <button type="button" class="btn btn-dark text-white me-2" data-bs-toggle="modal" data-bs-target="#createSubCategoryModal">
                        ...
                        <table class="table table-striped table-bordered table-hover">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-center align-middle" style="width:18%">Actions</th>
                                </tr>
                            </thead>
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="align-middle">{{ $doc->category->category_name ?? '—' }}</td>
                                    <td class="text-center align-middle">{{ $doc->language->language_name ?? '—' }}</td>
                                    <td class="align-middle">
                                        <small class="text-muted">{{ basename($doc->file_path) }}</small>
                                    </td>
                                    <td class="text-center align-middle">{{ $doc->created_at }}</td>
                                    <td class="text-center align-middle">
                                        {{-- Attributes --}}
                                        <button type="button" class="btn btn-warning btn-sm text-white attr-btn"
                                            data-id="{{ $doc->category_id }}"
                                            data-name="{{ $doc->category->category_name ?? '' }}"
                                            title="Attributes">
                                            <i class="bi bi-sliders"></i>
                                        </button>
                                        {{-- Edit --}}
                                        <button type="button" class="btn btn-primary btn-sm"
                                            onclick="openEditDoc({{ $doc->id }}, {{ $doc->category_id }}, {{ $doc->language_id ?? 'null' }})"
                                            title="Edit">
                                        ...
                                        {{-- Preview --}}
                                         <!-- {{-- Preview --}}
                                                        <button type="button" class="btn btn-info btn-sm text-white"
                                                            onclick="previewDoc({{ $doc->id }}, '{{ $doc->category->category_name ?? '' }} - {{ $doc->language->language_name ?? '' }}')"
                                                            title="Preview">
                                                            <i class="bi bi-eye"></i>
                                                        </button> -->
                                                {{-- Download --}}
                                                ...
                                                <!-- {{-- Delete --}}
                                                        <button type="button" class="btn btn-danger btn-sm" data-bs-toggle="modal"
                                                            data-bs-target="#deleteDocModal_{{ $doc->id }}" title="Delete">
                                            ...
                                            <div class="modal-dialog">
                                                <div class="modal-content">
                                                    ...
                                                    <div class="modal-body">
                                                        Delete <strong>{{ basename($doc->file_path) }}</strong>? This will also remove the file from storage.
                                                    </div>
                ...
                <div class="card p-2 pt-4">
                    <div class="card-body">
                        ...
                        <div id="attributeTableWrapper">
                            <div id="bulkDeleteBar" class="d-none mb-2 d-flex align-items-center gap-2">
                            ...
                            <table class="table table-striped table-bordered table-hover" id="attributeTable">
                                <thead>
                                    <tr>
                                        <th class="text-center align-middle" style="width:36px;">
                                            <input type="checkbox" id="selectAllAttrs" title="Select All">
                                        </th>
                                ...
                                <tbody id="attributeTableBody">
                                    <tr>
                                        <td colspan="8" class="text-center text-muted">Click "Attributes" on a sub category to load.</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="sub_category_id" id="docxSubCategoryId" class="form-select">
                            <option value="">Select Language first</option>
                        </select>
                        <small id="docxSubCategoryHint" class="text-muted d-none">Sub categories with an existing document for this language are hidden.</small>
                    </div>
                    <div class="mb-3">
                    ...
                    <div class="form-check mb-3">
                        <input type="checkbox" name="extract_attributes" id="extractAttributesCheck" class="form-check-input" value="1" checked>
                        <label class="form-check-label" for="extractAttributesCheck">
                            Extract &amp; save attributes from file variables
                        </label>
                    </div>
...
<div class="modal fade" id="updateDocModal" tabindex="-1">
    <div class="modal-dialog">
        <form id="updateDocForm" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <input type="hidden" name="document_id" id="updateDocId">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title"><i class="bi bi-pencil-square text-primary"></i> Update Document</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body">
                    <div class="mb-3">
                        <label class="form-label">Sub Category</label>
                        <select name="sub_category_id" id="updateSubCategoryId" class="form-select">
                    ...
                    <div class="mb-3">
                        <label class="form-label">Language <span class="text-danger">*</span></label>
                        <select name="language_id" id="updateLanguageId" class="form-select" required>
                            <option value="">Select Language</option>
                            @foreach(\App\Models\Language::where('is_active', 1)->orderBy('language_name')->get() as $lang)
                                <option value="{{ $lang->id }}">{{ $lang->language_name }}</option>
                            @endforeach
                    ...
                    <div class="mb-3">
                        ...
                        <input type="file" name="document" class="form-control" accept=".doc,.docx">
                        <small class="text-muted">Leave empty to keep existing file.</small>
                    </div>
                    <div class="form-check mb-3">
                        <input type="checkbox" name="extract_attributes" id="updateExtractCheck" class="form-check-input" value="1">
                        <label class="form-check-label" for="updateExtractCheck">
                            Extract &amp; save attributes from file variables
                        </label>
                    </div>
                </div>
...
<script>
    ...
    function loadAttributes(categoryId) {
        ...
        tbody.innerHTML = '<tr><td colspan="8" class="text-center"><div class="spinner-border spinner-border-sm text-primary"></div> Loading...</td></tr>';
        ...
        .then(function(response) {
            ...
            if (!data || data.length === 0) {
                tbody.innerHTML = '<tr><td colspan="8" class="text-center text-muted">No attributes found for this category.</td></tr>';
                return;
            }
            ...
            data.forEach(function(attr, i) {
                ...
                html += `<tr data-id="${attr.id}">
                    <td class="text-center align-middle">
                        <input type="checkbox" class="attr-checkbox" value="${attr.id}">
                    </td>
                    ...
                    <td class="text-center align-middle">
                        <span class="badge bg-secondary font-monospace copy-code" style="cursor:pointer;" title="Click to copy"
                            onclick="copyCode(this, '${attr.attribute_code_v2 ?? attr.attribute_code}')">
                            ${attr.attribute_code_v2 ?? attr.attribute_code}
                        </span>
                    </td>
            ...
            // Select all checkbox
            document.getElementById('selectAllAttrs').checked = false;
            document.getElementById('selectAllAttrs').onchange = function() {
            ...
            tbody.addEventListener('change', function(e) {
                if (e.target.classList.contains('attr-checkbox')) updateBulkBar();
            });
    ...
    function copyCode(el, code) {
        navigator.clipboard.writeText(code).then(function() {
            ...
            el.classList.replace('bg-secondary', 'bg-success');
            setTimeout(function() {
                el.textContent = original;
                el.classList.replace('bg-success', 'bg-secondary');
            }, 1200);
        });
    }
    ...
    function bulkDelete() {
        const ids = [...document.querySelectorAll('.attr-checkbox:checked')].map(cb => cb.value);
        if (!ids.length) return;
        if (!confirm('Delete ' + ids.length + ' selected attribute(s)? This cannot be undone.')) return;

        fetch('{{ route("attribute.bulk_delete") }}', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': '{{ csrf_token() }}' },
    ...
    document.getElementById('uploadDocxForm').addEventListener('submit', function(e) {
        ...
        // Explicitly set extract_attributes since unchecked checkboxes are not included in FormData
        formData.set('extract_attributes', document.getElementById('extractAttributesCheck').checked ? '1' : '0');
    ...
    // Sub category filter in upload modal based on language + already uploaded combos
    const allSubCategories = @json($category->map(fn($s) => ['id' => $s->id, 'name' => $s->category_name]));
    const uploadedCombos   = @json($uploadedCombos);

    document.getElementById('docxLanguageSelect').addEventListener('change', function() {
        const langId   = this.value;
        const select   = document.getElementById('docxSubCategoryId');
        const hint     = document.getElementById('docxSubCategoryHint');
        ...
        allSubCategories.forEach(function(sub) {
            const combo = sub.id + '_' + langId;
            if (!uploadedCombos.includes(combo)) {
        ...
        if (added === 0) {
            select.innerHTML = '<option value="">All sub categories already have a document for this language</option>';
        }

        hint.classList.toggle('d-none', added === allSubCategories.length);
    });

    // Update document modal
    function openEditDoc(docId, categoryId, languageId) {
        document.getElementById('updateDocId').value = docId;
        document.getElementById('updateDocForm').action = '{{ url("documents/update") }}/' + docId;

        // Pre-select sub category
        const subSel = document.getElementById('updateSubCategoryId');
        subSel.value = categoryId || '';

        // Pre-select language
        const langSel = document.getElementById('updateLanguageId');
        langSel.value = languageId || '';

        document.getElementById('updateExtractCheck').checked = false;
    ...
    document.getElementById('updateDocForm').addEventListener('submit', function(e) {
        ...
        const formData = new FormData(this);
        formData.set('extract_attributes', document.getElementById('updateExtractCheck').checked ? '1' : '0');
