    ...
    <section class="section">
        <div class="card shadow-sm border-0">
            <div class="card-body p-4">
                <form action="{{ route('notification-templates.update', $template->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-select @error('notification_type') is-invalid @enderror" id="notification_type" name="notification_type" required>
                                @foreach($types as $key => $val)
                                    <option value="{{ $key }}" {{ old('notification_type', $template->notification_type) === $key ? 'selected' : '' }}>{{ $val }}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('message') is-invalid @enderror" id="message" name="message">{{ old('message', $template->message) }}</textarea>
                        @error('message')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-select @error('status') is-invalid @enderror" id="status" name="status" required>
                                <option value="1" {{ old('status', $template->status ? '1' : '0') === '1' ? 'selected' : '' }}>Active</option>
                                <option value="0" {{ old('status', $template->status ? '1' : '0') === '0' ? 'selected' : '' }}>Inactive</option>
                            </select>
                            @error('status')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
