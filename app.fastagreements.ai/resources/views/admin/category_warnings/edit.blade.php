    ...
    <div class="pagetitle mb-4">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb" style="background: none; padding: 0;">
                ...
                <li class="breadcrumb-item active" aria-current="page">Edit Warning</li>
            </ol>
        </nav>
        <div class="row align-items-center">
            <div class="col-md-6 pt-2">
                <h1 class="fw-bold text-dark mb-1" style="font-size: 28px;">Edit Category Warning</h1>
                <p class="text-muted mb-0" style="font-size: 14px;">Modify the details of this warning for <strong>{{ $category->category_name }}</strong>.</p>
            </div>
            ...
            <div class="col-lg-12">
                <div class="card border-0 shadow-sm" style="border-radius: 10px;">
                    <div class="card-body p-4 pb-0">
                        <form action="{{ route('category-warnings.update', $warning->id) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-4">
                                    ...
                                    <select class="form-select @error('language_id') is-invalid @enderror" id="language_id" name="language_id" required>
                                        ...
                                        @foreach($languages as $language)
                                            <option value="{{ $language->id }}" {{ old('language_id', $warning->language_id) == $language->id ? 'selected' : '' }}>{{ $language->language_name }}</option>
                                        @endforeach
                                    </select>
                                    @error('language_id')
                                ...
                                <div class="col-md-6 mb-4">
                                    ...
                                    <input type="text" class="form-control @error('title') is-invalid @enderror" id="title" name="title" value="{{ old('title', $warning->title) }}" placeholder="Enter warning title" required>
                                    @error('title')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                            ...
                            <div class="row">
                                <div class="col-md-12 mb-4">
                                    ...
                                    <textarea class="form-control tinymce-editor @error('description') is-invalid @enderror" id="description" name="description" placeholder="Write warning detail...">{!! old('description', $warning->description) !!}</textarea>
                                    @error('description')
                            ...
                            <div class="row align-items-center">
                                <div class="col-md-6 mb-4">
                                    ...
                                    <input type="number" class="form-control @error('display_order') is-invalid @enderror" id="display_order" name="display_order" value="{{ old('display_order', $warning->display_order) }}" required>
                                    @error('display_order')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="col-md-6 mb-4 d-flex align-items-center mt-3 justify-content-md-start">
                                    <div class="form-check form-switch">
                                        <input class="form-check-input" type="checkbox" id="status" name="status" value="1" {{ old('status', $warning->status) ? 'checked' : '' }}>
